...
def simple_detector():
    ...
    def apply_detection(control_path: str, current_path: str, metadata: dict):
        ...
        result = (((current_dataset[feature_columns].quantile(0.5) / control_dataset[feature_columns].quantile(0.5) - 1)  * 100)
                    .rename(lambda x: x + '__median').to_dict() |
                ((current_dataset[feature_columns].quantile(0.5) / control_dataset[feature_columns].quantile(0.5) - 1) * 100)
    ...
    def send_metrics(metrics: typing.Dict[str, float]):
        for feature_metric, value in metrics.items():
            ...
            reply = requests.post('http://monitor:5445/drift', params={
                'feature': feature,
                'metric': metric,
                'value': value
            })
